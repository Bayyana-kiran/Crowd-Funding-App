...
contract Crowdfunding {
    ...
    event CampaignUpdated(CampaignState newState);
    ...
    function checkAndUpdateCampaignState() internal {
        if (state == CampaignState.Active) {
            ...
            emit CampaignUpdated(state);
        }
    }

    function fund(uint256 _tierIndex) public payable campaignOpen notPaused {
        require(_tierIndex < tiers.length, "Invalid Tier");
        require(msg.value >= tiers[_tierIndex].amount, "Insufficient amount");
    ...
    function removeTier(uint256 _index) public onlyOwner {
        ...
        tiers[_index] = tiers[tiers.length - 1]; // Move last tier to deleted slot
        tiers.pop();
